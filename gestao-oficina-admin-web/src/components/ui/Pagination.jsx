...
export function Pagination({
  ...
  return (
    <div
      className={`flex flex-col gap-3 border-t border-ink-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between ${className}`.trim()}
    >
      ...
      <nav className="flex flex-wrap items-center gap-1" aria-label="Paginação">
        ...
        {pages.map((p, idx) =>
          p === '…' ? (
            <span key={`e-${idx}`} className="px-1 text-xs text-ink-400">
              …
            </span>
          ) : (
            <PageButton
